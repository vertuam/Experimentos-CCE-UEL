...
from gerador.april.generation import AttributeGenerator
from gerador.april.generation import NoneAnomaly
from gerador.april.generation.drift import NoneDrift
...
class EventLogGenerator(object):
    ...
    def generate(self,
        ...
        none_drift = NoneDrift()
        none_drift.activities = activities
        none_drift.graph = self.likelihood_graph
        none_drift.attributes = self.event_attributes
        for drift in drifts:
            drift.activities = activities
            drift.graph = self.likelihood_graph
            drift.attributes = self.event_attributes
        ...
        event_log.attributes['generation_parameters'] = dict(
            ...
            drifts=[a.json for a in drifts],
            drift_p=drift_p,
            drift_type_p=drift_type_p,
            activity_dependency_p=activity_dependency_p,
            attribute_dependency_p=attribute_dependency_p,
            probability_variance_max=probability_variance_max,
            seed=int(seed)
    ...
    def plot_likelihood_graph(self, file_name=None, figsize=None):
        from gerador.april.utils import microsoft_colors
