...
from gerador.april.fs import EVENTLOG_CACHE_DIR
from gerador.april.fs import EVENTLOG_DIR
...
class EventLog(object):
        ...
        for case_id, case in enumerate(self.cases):
            ...
            for event_pos, event in enumerate([start_event] + case.events + [end_event]):
                if case.attributes['label'] == 'normal':
                    ...
                    label = 'anomalous'
                    anomaly_type = case.attributes['label']['anomaly']
                    anomaly_description = case.attributes['label']['attr']
                frames.append({
                    'case_id': case_id,
    ...
    def save(self, name, p=0.0, number=1):
        ...
        #self.save_json(EVENTLOG_DIR / f'json_new/{base_name}.json.gz')
        self.save_csv(EVENTLOG_DIR / f'csv_new/{base_name}.csv')
